...
static int
in_limit(int y, int x)
{
    return (y >= 0 && y < map_height && x >= 0 && x < map_width);
}

// get floortype of vector2
static enum floortype
floor_vect(struct vector2 v)
{
...
static int
squeezed(int old_row, int old_col, int row, int col)
{
...
// fill matrice of 0
static void
zero_fill(int **mat, int height, int width)
{
...
static void
init_point(struct point *p, float y, float x)
{
    p->parent_x = x;
    p->parent_y = y;
    p->h = 0.0;
...
static void
init_mat_point(int height, int width)
{
    for (int i = 0; i < height; ++i)
    {
        for (int j = 0; j < width; ++j)
...
void
init_los(int *dy, int *sy, int *dx, int *sx)
{
    if (*dy < 0)
    {
        *dy *= -1;
...
int
line_of_sight(int parent_y, int parent_x, int y, int x)
{
    int sy = 1, sx = 1, f = 0;
    int dy = y - parent_y;
    int dx = x - parent_x;
    ...
    if (dx >= dy)
    {
        while (parent_x != x)
        {
            ...
            {
                if (blocked(
                        parent_y + ((sy - 1) / 2), parent_x + ((sx - 1) / 2)))
                    return 0;
            ...
            if (f != 0
                && blocked(
                       parent_y + ((sy - 1) / 2), parent_x + ((sx - 1) / 2)))
                return 0;
            if (!dy && blocked(parent_y, parent_x + ((sx - 1) / 2))
                && blocked(parent_y - 1, parent_x + ((sx - 1) / 2)))
                return 0;
            parent_x += sx;
        }
    }
        ...
        while (parent_y != y)
        {
            ...
            if (f != 0
                && blocked(
                       parent_y + ((sy - 1) / 2), parent_x + ((sx - 1) / 2)))
                return 0;
            if (!dx && blocked(parent_y + ((sy - 1) / 2), parent_x)
                && blocked(parent_y + ((sy - 1) / 2), parent_x - 1))
                return 0;
            parent_y += sy;
        }
    }
...
static double
get_c(int row, int col, int finish_x, int finish_y)
{
...
void
ComputeCost(int old_y, int old_x, int y, int x)
{
    double new_g;
...
static void
get_path(struct vector *path, int row, int col)
{
    int row_tmp;
    if (!path)
    {
    ...
    {
        path = vector_insert(
            path, 0, make_vec2((float)(row) / SCALE, (float)(col) / SCALE));
...
static void
explore(struct vector *path, int row, int col, struct vector *open)
{
    if (goal)
        return;
...
static void
explore_all(struct vector *path, struct vector *open)
{
...
void
fill_my_map(struct map *map, int height, int width)
{
    for (int i = 0; i < height; ++i)
    {
        for (int j = 0; j < width; ++j)
...
find_path(struct map *map)
{
    ...
    if (!finish || blocked_vec(start) || arrived_vec(start)) // initial
                                                             // incorrect
    ...
    struct vector *open
        = vector_append(NULL, make_vec2(row_static, col_static));
