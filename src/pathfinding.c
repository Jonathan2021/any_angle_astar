...
static void explore(struct map *map, int row, int col, double factor)
{
    if(goal)
        return;
...
static char **create_matrix(struct map *map, struct vector *path)
{
...
static void print_map(struct map *map, struct vector *path)
{
