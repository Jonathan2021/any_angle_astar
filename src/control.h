...
enum floortype
map_get_floor(struct map *map, int x, int y);
float
map_get_start_x(struct map *map);
float
map_get_start_y(struct map *map);
void
map_delete(struct map *map);

struct car *
car_new(struct map *map);
struct car *
car_clone(struct car *car);
void
car_delete(struct car *car);
enum status
car_move(struct car *car, enum move move);

struct vector2 *
vector2_new(void);
struct vector2 *
vector2_clone(struct vector2 *vector);
void
vector2_delete(struct vector2 *vector);
